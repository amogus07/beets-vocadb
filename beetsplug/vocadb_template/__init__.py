...
def get_vocadb_plugin(name: str, base_url: str, api_url: str, subcommand: str):
    class _VocaDBPluginTemplate(BeetsPlugin):
        ...
        DEFAULT_PREFER_ROMAJI: bool = False
        DEFAULT_TRANSLATED_LYRICS: bool = False
        DEFAULT_INCLUDE_FEATURED_ALBUM_ARTISTS: bool = False
        DEFAULT_VA_STRING: str = "Various artists"

        vocadb_config: VocaDBConfig

        def __init__(
                self,
                vocadb_config: VocaDBConfig = VocaDBConfig(
                    name=name,
                    base_url=base_url,
                    api_url=api_url,
            ...
            self.config.add(
                    {
                        "source_weight": self.DEFAULT_SOURCE_WEIGHT,
                        "prefer_romaji": self.DEFAULT_PREFER_ROMAJI,
                        "translated_lyrics": self.DEFAULT_TRANSLATED_LYRICS,
                        "include_featured_album_artists": self.DEFAULT_INCLUDE_FEATURED_ALBUM_ARTISTS,
                        "va_string": self.DEFAULT_VA_STRING
                    }
            )
            self.vocadb_config = vocadb_config
